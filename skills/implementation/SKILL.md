...
- **Confirm isolation first.** Ask the user whether to use worktrees (unless they've stated a preference). Declining → serial in-place execution.
- **Execution:** topological waves from the plan's DAG; independent steps in a wave run in parallel as `general-purpose` subagents (worktree mode only).
...
3. **Confirm isolation, then lay out worktrees.** Before creating anything, confirm the execution mode with the user (unless they've already stated a preference — see workflow step). Default and recommended: isolated worktrees. If confirmed: all work lives under `<repo>/.worktrees/<plan>/` (gitignored). A long-lived **consolidated** worktree at `.worktrees/<plan>/main` (branch `impl/<plan>/main`) is where every step's changes accumulate. Each parallel subagent gets its own **step** worktree at `.worktrees/<plan>/step-<N>-<slug>` (branch `impl/<plan>/step-<N>-<slug>`), forked from the consolidated worktree's current tip. The user's main checkout is never touched during the run. All worktree lifecycle is delegated to [scripts/worktree.sh](#worktree-management) — never hand-roll `git worktree`. If the user declines worktrees: serial, in-place execution in the main checkout — see [No-worktree fallback](references/worktrees.md#no-worktree-fallback).
...
- [ ] **Confirm the execution mode.** If the user already stated a preference (their message, or project CLAUDE.md), honor it without asking. Otherwise ask via `AskUserQuestion`: **(a) Isolated worktrees (recommended)** — parallel waves, main checkout untouched, one staged diff at hand-off; **(b) Work in place** — serial execution in the main checkout, changes accumulate as staged edits (still never committed), no parallelism. If they pick (b), skip every `worktree.sh` call below and run steps serially in the main checkout — see [No-worktree fallback](references/worktrees.md#no-worktree-fallback).
- [ ] **Initialize the consolidated worktree** *(worktree mode only)*. Run `bash ~/.claude/skills/implementation/scripts/worktree.sh init <plan-slug>` — creates `<repo>/.worktrees/<plan>/main` on branch `impl/<plan>/main`, saves `BASE_REF`, ensures `.worktrees/` is in `.gitignore`. The script prints the consolidated path on stdout — capture it. See [references/worktrees.md](references/worktrees.md).
- [ ] **For each wave** (see [Wave execution](#wave-execution)). *In-place mode: there are no waves — run every step serially in dependency order, orchestrator-executed, pausing for the user at each step boundary. See [No-worktree fallback](references/worktrees.md#no-worktree-fallback).*
